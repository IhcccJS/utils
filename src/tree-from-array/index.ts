 ...
 * @member {number} config.deep 根节点默认深度，默认 `0`；设置 `null` 将不添加 `deep` 字段
...
function treeFromArray(array: TNode[], config?: TConfig): TNode[] {
  ...
  return array.filter(filterFn).map((item) => {
    const children = treeFromArray(array, {
      ...
      deep: deep === null ? null : deep + 1,
    });

    let node;
    if (deep === null) {
      node = Object.assign(item, children.length > 0 ? { [childrenKey]: children } : {});
    } else {
      node = Object.assign(item, children.length > 0 ? { deep, [childrenKey]: children } : { deep });
    }
